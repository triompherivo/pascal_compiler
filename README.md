Hello, This is one of a projects i managed to create with free ChatGPT with reasoning.
It's a Pascal language compiler and interpreter written in RUST
It supports type (integer, double,float, String) , loop (for,while,repeat ...until) , branch (if) , record (struct)
function and procedure with parameters
writeln and readln and file I/O 
and last but not least recursion
